...
class ScrollArea(QScrollArea):
    ...
    def wheelEvent(self, e: QWheelEvent):
        ...
        self.lastWheelEvent = QWheelEvent(e.pos(), e.globalPos(), e.pixelDelta(), e.angleDelta(), e.buttons(), e.modifiers(), e.phase(), e.source())

        # getTranslation the number of steps
        self.stepsTotal = self.fps * self.duration / 1000

        # getTranslation the moving distance corresponding to each event
    ...
    def __subDelta(self, delta, stepsLeft):
        """ getTranslation the interpolation for each step """
        m = self.stepsTotal / 2
